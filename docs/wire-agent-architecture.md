...
The token and cost cells in the log deserve a note. The command writes them as `n/a`. After the turn ends, a hook in the plugin reads the measured usage from the Claude Code transcript and fills them in. The model never estimates them.
...
For a concrete example, look at this repo. The `status.md` for release `01-looker-to-omni` records orchestrated mode, a claim by Mark Rittman in session `7bb027f9` on `main`, taken at 21:56 on 6 September 2026, and one parked decision, PD-1, asking which Omni shared model the migration should write to. The release execution log has one row so far: the activation of the `release-director` skill.

## 3. Lane agents

A lane is one specialist agent doing one scoped job, in its own directory, with its own state file. Lanes are flat: a lane never starts agents of its own.
...
All 13 definitions set `model: claude-opus-4-8`. A director who sets `model_tier: economy` in the budget gets lanes dispatched on a lower-tier model instead; the spec does not name which one. The consolidation pass runs either way. The choice of model changes how much that pass finds, not whether it happens.
...
The orchestrator also sets `WIRE_INVOKED_BY=lane` in the lane's environment, so both the telemetry event and the log row record what invoked the run.
...
Parallelism comes from the shape of the release graph rather than from counting items. If two artifacts are runnable and neither depends on the other, they become two lanes, up to the `lanes_max` budget setting, which defaults to 4. Anything beyond that queues in dependency order. Some artifacts are interactive, meaning their generate step needs input from the person as it goes; mockups on a dashboard-first release is the usual case. Those run in the foreground with the director, never as a lane. The orchestrator never polls a lane to ask how it is getting on. It reads the state files. A lane that has not written its state file for 30 minutes is treated as stalled, and its remaining items can be handed to a fresh lane.

Three agents split big jobs into batches. The dbt developer does it when any layer has more than 5 models: batches of 5, up to 8 per layer, with staging finished before integration starts and integration before warehouse. The semantic-layer developer batches by explore, 3 at a time up to 6 batches. The migration specialist batches source tables, 10 at a time up to 8 batches. Each batch counts as a lane against `lanes_max`, and the orchestrator only marks the artifact complete once every batch in the wave has finished.

<p align="center"><img src="images/wire-agent-architecture-3-lanes.png" alt="The orchestrating session computing the runnable set, dispatching three lanes within the budget, and consolidating their reports" width="720"></p>
<p align="center"><em>Diagram 3. The orchestrating session working out what is runnable, dispatching three lanes within the budget, and consolidating their reports.</em></p>
...
Outside orchestrated mode the same agents are still used, just differently. A single generate or validate command checks whether its specialist's definition exists and, if so, hands the work to that subagent. In that mode the subagent updates `status.md` itself, exactly as it did in 3.x.

<p align="center"><img src="images/wire-agent-architecture-4-lane-internals.png" alt="One dbt-developer lane running /wire:dbt-generate through the gate, the steps, validate and its state file, with the skills it loads, the tools it calls and what it writes" width="720"></p>
<p align="center"><em>Diagram 4. One dbt-developer lane running <code>/wire:dbt-generate</code>: the gate, the steps, validate and the state file, plus the skills it loads, the tools it calls and what it writes.</em></p>

## 4. Wire commands

The installed 4.0.0 build has 331 command files under `commands/`: 92 generate, 87 validate, 82 review, and 70 others covering engagement setup, sessions, status, migration tooling and utilities. Each is a Claude Code slash command, which is to say a markdown file that Claude Code loads when someone types or invokes `/wire:<name>`, with the release folder arriving in `$ARGUMENTS`.

Open one and you will find the same shape every time. The front-matter carries a description and an argument hint, plus a `model:` line on the 7 commands that are pinned to a model. Then come the user input and path configuration (`.wire/` in the repo, `${CLAUDE_PLUGIN_ROOT}/specs/` for shared specs), a tracing section that only does anything when `WIRE_TRACE=true`, and, on 82 of the 92 generate commands, an automatic validation section: generate runs its own validate step and folds the result into its output, unless the spec sets `auto_validate: false` because validate would run real code or query a live system. After that is the workflow specification proper, with its own front-matter (`command`, `artifact`, `domain`, `release_types`, `action_type`, `logs_execution`, `inputs`, `preconditions` and `delegates_to`), a pointer to the auto-delegation utility for its specialist agent, the numbered steps, the embedded templates and the list of output files. The file ends with the execution logging section.
...
The build ships 69 skills under `skills/`, each a `SKILL.md` with a name, a description and a list of triggers. Claude Code loads a skill when the conversation matches its triggers or when someone invokes it by name. Skills do not write artifacts. They shape how the session and the lanes behave, and each activation adds a `skill | <id> | activated` row to the execution log.

They fall into four groups. The control skills run the show: `engagement-context` loads the engagement and release state at the start of a conversation; `release-director` is the orchestrating session's operating procedure, the thing that resolves mode, release and claim, reports parked decisions, works out the runnable set and dispatches; and `fathom-sync` pulls new client call transcripts once per session where a client domain has been set. The domain knowledge skills (`dbt-development`, `droughty`, `lookml-content-authoring`, `omni`, `dagster`, `fivetran`, `airbyte`, `metabase`, `snowflake-development` and the rest) carry naming conventions, validation rules and platform procedures. They are loaded into a lane through the `skills` list in its agent definition, and into the main session whenever you work outside Wire commands. The `omni` skill wraps the 9 upstream skills from `exploreomni/omni-agent-skills`, which are installed separately. The writing standards are the `technical-writing` skill and the `plain-language` output style, which is forced on for the plugin. Finally, `wire-release`, `wire-usage-analysis` and `research` look after the framework itself and save research findings under `.wire/research/sessions/`.
...
Claude Code's permission mode decides what the session and its lanes are allowed to do. For unattended runs against live systems the recommended setting is `acceptEdits` plus an explicit allowlist in `.claude/settings.json`. This repo ships one with 8 Bash patterns: git clone, git inside the LookML snapshot, git ls-remote, omni, python3, date, mkdir and ls. Anything outside the list fails visibly in the turn output, which is what you want. `--dangerously-skip-permissions` is not used against live systems.

You do not need a person at the keyboard. `claude -p "<text>"` runs a single turn and `claude -p -c "<text>"` continues the same conversation. This repo's `harness/run_test.sh` uses that to send turns 1 to 5 of the migration test script, with `--output-format stream-json` rendered readable by `harness/format_stream.py`, everything appended to `harness_run.log`, and a gate pattern checked against each turn's final text before the next one is sent. Turn 6, the cutover, is deliberately not in the harness. That is a ruling a person gives in an interactive session. The record does not care whether a run was headless or interactive; it looks the same either way.
...
Two other runtimes are worth knowing about. Gemini CLI runs the same commands but has no skills, agents or per-command model field, so it is manual mode only. The VS Code extension (`wire-vscode`, version 3.10.19) is a sidebar over the `.wire/` files with a command picker; it does not take part in orchestration.
...
There are two sets. The detailed set is the one embedded above. The simple set drops the titles, uses fewer boxes and one line per tier, and is meant for slides and summaries.

| Item | Location |
|---|---|
| Editable canvas, 2 pages ("Detailed" and "Simple"), 4 artboards each in a 2 by 2 grid | https://claude.ai/code/artifact/fe49017f-7a8e-49ad-a1c4-4cb9e19b336f |
| Artboard sources and layout | `docs/diagrams/wire-agent-architecture/`: detailed `Main.dc.html` (titled Architecture), `DirectorOrchestrator.dc.html`, `LaneDispatch.dc.html`, `LaneInternals.dc.html`; simple `ArchitectureSimple.dc.html`, `DirectorOrchestratorSimple.dc.html`, `LaneDispatchSimple.dc.html`, `LaneInternalsSimple.dc.html`; plus `canvas.json` and `logo-black-blue.svg` |
| PNG exports, detailed, 2880 by 1800 | `docs/images/wire-agent-architecture-1-layers.png`, `-2-director.png`, `-3-lanes.png`, `-4-lane-internals.png` |
| PNG exports, simple, 2880 by 1800 | `docs/images/wire-agent-architecture-1-layers-simple.png`, `-2-director-simple.png`, `-3-lanes-simple.png`, `-4-lane-internals-simple.png` |

To change a diagram, edit its `.dc.html` source and re-run `/design` on the folder, or edit it in the canvas and save. Re-export the PNG afterwards so the images here match the canvas, and keep the artboard names as they are between revisions.
